...
class EDD_Plugin_Updater_Admin {
	...
	function license_menu() {
		add_theme_page(
			__( 'Kuorinka Plus License', 'kuorinka-plus' ),
			__( 'Kuorinka Plus License', 'kuorinka-plus' ),
			'manage_options',
	...
	function license_page() {
		...
		if ( ! $license ) {
			$message = __( 'Enter your plugin license key.', 'kuorinka-plus' );
		} else {
		...
		?>
		<div class="wrap">
			<h2><?php _e( 'Plugin License', 'kuorinka-plus' ); ?></h2>
					...
					<tbody>
						...
						<tr valign="top">
							<th scope="row" valign="top">
								<?php _e( 'License Key', 'kuorinka-plus' ); ?>
							</th>
							<td>
						...
						<tr valign="top">
							<th scope="row" valign="top">
								<?php _e( 'License Action', 'kuorinka-plus' ); ?>
							</th>
							<td>
								<?php
								wp_nonce_field( $this->plugin_slug . '_nonce', $this->plugin_slug . '_nonce' );
								if ( 'valid' == $status ) { ?>
									<input type="submit" class="button-secondary" name="<?php echo $this->plugin_slug; ?>_license_deactivate" value="<?php esc_attr_e( 'Deactivate License', 'kuorinka-plus' ); ?>"/>
								<?php } else { ?>
									<input type="submit" class="button-secondary" name="<?php echo $this->plugin_slug; ?>_license_activate" value="<?php esc_attr_e( 'Activate License', 'kuorinka-plus' ); ?>"/>
								<?php }
								?>
							</td>
						</tr>
	...
	function check_license() {
		...
		if ( !isset( $license_data->license ) ) {
			$message = __( 'License status is unknown.', 'kuorinka-plus' );
			return $message;
		}
		...
		if ( isset( $license_data->expires ) ) {
			...
			$renew_link = '<a href="' . esc_url( $this->remote_api_url) . '">' . __( 'Renew?', 'kuorinka-plus' ) . '</a>';
		}
		...
		// If unlimited
		if ( 0 == $license_limit ) {
			$license_limit = __( 'unlimited', 'kuorinka-plus' );
		}

		if ( $license_data->license == 'valid' ) {
			$message = __( 'License key is active.', 'kuorinka-plus' ) . ' ';
			if ( $expires ) {
				$message .= sprintf( __( 'Expires %s.', 'kuorinka-plus' ), $expires ) . ' ';
			}
			if ( $site_count && $license_limit ) {
				$message .= sprintf( __( 'You have %1$s / %2$s sites activated.', 'kuorinka-plus' ), $site_count, $license_limit );
			}
		} else if ( $license_data->license == 'expired' ) {
			if ( $expires ) {
				$message = sprintf( __( 'License key expired %s.', 'kuorinka-plus' ), $expires );
			} else {
				$message = __( 'License key has expired.', 'kuorinka-plus' );
			}
			if ( $renew_link ) {
				$message .= ' ' . $renew_link;
			}
		} else if ( $license_data->license == 'invalid' ) {
			$message = __( 'License keys do not match.', 'kuorinka-plus' );
		} else if ( $license_data->license == 'inactive' ) {
			$message = __( 'License is inactive.', 'kuorinka-plus' );
		} else if ( $license_data->license == 'disabled' ) {
			$message = __( 'License key is disabled.', 'kuorinka-plus' );
		} else if ( $license_data->license == 'site_inactive' ) {
			// Site is inactive
			$message = __( 'Site is inactive.', 'kuorinka-plus' );
		} else {
			$message = __( 'License status is unknown.', 'kuorinka-plus' );
		}
